function [BPE, rho, z] = buoyancy_potential_energy_approx(rho, z, zint)
% Buoyancy Potential Energy (BPE) from approximate energetic formulations
...
it = find(z == zint, 1); 
dz = diff(z); 
rho_eq = rho(it); % Density at the reference height zint
